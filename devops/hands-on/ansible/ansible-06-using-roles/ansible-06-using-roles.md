...
```bash
$ ansible _test_server -a "systemctl stop httpd" -b --key-file ~/mykey.pem
or
ansible test_server -a "autoremove -f httpd" --keyfile "~/mykey.pem"
or
ansible _test_server -b -m yum -a "name=httpd state=absent"  --key-file "~/northvirginia.pem"
